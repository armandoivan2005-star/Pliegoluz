...
export default async function ReaderPage({ params }: PageProps<"/leer/[slug]/[chapter]">) {
  ...
  return <ReaderShell bookSlug={book.slug} bookTitle={book.title} chapterNumber={chapter.number} chapterRoman={chapter.roman} chapterTitle={chapter.title} paragraphs={getChapterParagraphs(chapter)} totalChapters={book.chapters.length} isPreview={!chapter.contentMarkdown?.trim()} />;
}
