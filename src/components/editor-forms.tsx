...
import type { EditorBook, EditorChapter, PublicationStatus } from "@/lib/editorial";
  ...
  return (
    <form action={action} className="space-y-7">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-medium">Slug público<input className={inputClass} name="slug" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" defaultValue={book?.slug} placeholder="nombre-del-libro" /></label>
      </div>
      <label className="block text-sm font-medium">Subtítulo<input className={inputClass} name="subtitle" maxLength={220} defaultValue={book?.subtitle ?? ""} placeholder="Edición, volumen o frase descriptiva" /></label>
      <div className="grid gap-5 sm:grid-cols-2">
  ...
  return (
    <form action={action} className="space-y-7">
      <div className="grid gap-5 sm:grid-cols-[150px_1fr_190px]">
        ...
        <label className="text-sm font-medium">Título<input className={inputClass} name="title" required maxLength={180} defaultValue={chapter?.title} placeholder="Título del capítulo" /></label>
      ...
      <label className="block text-sm font-medium">Contenido<span className="ml-2 font-normal text-black/45">Separa los párrafos con una línea vacía</span><textarea className={`${inputClass} min-h-[560px] resize-y font-serif text-base leading-7`} name="content_markdown" spellCheck defaultValue={chapter?.content_markdown ?? ""} placeholder="Escribe o pega aquí el contenido del capítulo…" /></label>
